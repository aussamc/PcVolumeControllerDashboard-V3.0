...
/// Remote API (VoicemeeterRemote64.dll).  The DLL is loaded dynamically from
/// the VoiceMeeter install directory, located via the Windows registry.
///
/// Target key format:
...
/// VoiceMeeter Gain parameter range: −60.0 to +12.0 dB
/// </summary>
internal sealed class VoiceMeeterService : IAudioBackend
{
    // ── Constants ────────────────────────────────────────────────────────────
    ...
    // Resolved function pointers (null until DLL is loaded).
    ...
    // Background availability poll timer.
    ...
    /// </param>
    public VoiceMeeterService(Action<string>? logger = null)
    {
        _log = logger ?? (_ => { });
    }
    ...
    public void Initialise()
    {
        ...
        if (!_dllFound)
        {
            _log("VoiceMeeterService: DLL not found. VoiceMeeter is not installed.");
    ...
    private bool TryLoadDll()
    {
        ...
        if (dllPath == null)
        {
            _log("VoiceMeeterService: Could not locate install directory in registry.");
            return false;
        }
        ...
        {
            _log($"VoiceMeeterService: Failed to load '{dllPath}': {ex.Message}");
            return false;
        }
        ...
        {
            _log($"VoiceMeeterService: Failed to bind API functions: {ex.Message}");
            return false;
        }

        _log($"VoiceMeeterService: DLL loaded from '{dllPath}'.");
        return true;
    }
    ...
    private void TryLogin()
    {
        ...
        try
        {
            ...
            if (_loggedIn)
            {
                ...
                _log($"VoiceMeeterService: Logged in (type={_vmType}).");
            }
            else
            {
                SetAvailability(false);
                _log($"VoiceMeeterService: Login returned {result} — VoiceMeeter may not be running.");
            }
        }
        catch (Exception ex)
        {
            _loggedIn = false;
            SetAvailability(false);
            _log($"VoiceMeeterService: Login exception: {ex.Message}");
        }
    }
    ...
    private void PollAvailability(object? _)
    {
        ...
        try
        {
            ...
            {
                // VM went offline — logout so we can re-login later.
                TryLogout();
            }
            else if (nowAvailable && !_loggedIn)
            {
                // VM came back — try logging in.
                TryLogin();
        ...
        {
            _log($"VoiceMeeterService: Poll error: {ex.Message}");
            SetAvailability(false);
        }
    }
    ...
    public List<AudioTargetItem> GetAvailableTargets()
    {
        var list = new List<AudioTargetItem>();
    ...
    /// <summary>
    /// Converts a dashboard target key to the VoiceMeeter Gain parameter name,
    /// or null if the key is not a VoiceMeeter key.
    /// </summary>
    ...
    // ── Utility: key detection (used by MainWindow) ───────────────────────────
